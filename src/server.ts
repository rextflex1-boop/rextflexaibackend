...
app.get('/v1/sessions/:id',auth,async(req,res)=>{const owner=await sessionOwner(req.params.id);if(owner!==req.user.id)return res.status(404).json({error:'Not found'});res.json({messages:await sessionMessages(req.params.id,req.user.id)})});
app.patch('/v1/sessions/:id',auth,async(req,res)=>{if(req.body.title)await renameSession(req.params.id,req.user.id,req.body.title);if(typeof req.body.isPublic==='boolean')await setSessionPublic(req.params.id,req.user.id,req.body.isPublic);res.json({ok:true})});
app.delete('/v1/sessions/:id',auth,async(req,res)=>{await deleteSession(req.params.id,req.user.id);res.json({ok:true})});
app.get('/v1/share/:id',async(req,res)=>{const x=await publicSession(req.params.id);if(!x)return res.status(404).json({error:'Not found'});res.json(x)});
...
app.delete('/v1/personas/:id',auth,async(req,res)=>{await deletePersonaDb(req.params.id,req.user.id);res.json({ok:true})});
...
app.get('/v1/files/:id',auth,async(req,res)=>{const f=await getFile(req.params.id,req.user.id);if(!f)return res.status(404).json({error:'Not found'});res.setHeader('Content-Type',f.mimeType);res.setHeader('Content-Disposition',`attachment; filename="${String(f.fileName).replace(/"/g,'')}"`);res.send(f.data)});
...
app.get('/v1/agent/tasks/:id',auth,async(req,res)=>{const x=await getTask(req.params.id,req.user.id);if(!x)return res.status(404).json({error:'Not found'});res.json(x)});
app.post('/v1/agent/tasks/:id/approve',auth,async(req,res)=>{try{res.json(await approve(req.params.id,req.user.id));}catch(e:any){res.status(400).json({error:e.message})}});
app.post('/v1/agent/tasks/:id/cancel',auth,async(req,res)=>{try{res.json(await cancel(req.params.id,req.user.id));}catch(e:any){res.status(400).json({error:e.message})}});
