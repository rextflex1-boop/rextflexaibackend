...
export async function verify(req:any){const h=String(req.headers.authorization||'');if(!h.startsWith('Bearer '))return null;try{const p:any=jwt.verify(h.slice(7),secret());return await userById(p.sub);}catch{return null}}
function issue(user:any){const accessToken=jwt.sign({sub:user.id},secret(),{expiresIn:'30d'});return {accessToken,user};}
